...
authDivider.innerHTML = "<span>ou continue com e-mail</span>";

if (authForm) {
    authForm.before(googleLoginButton, authDivider);
}

googleLoginButton.addEventListener("click", async () => {
    if (!supabase) {
        authMessage.textContent = "A autenticação ainda está carregando. Tente novamente.";
        return;
    }

    googleLoginButton.disabled = true;
    authMessage.textContent = "Abrindo o Google...";

    try {
        sessionStorage.setItem("clipai_abrir_editor_google", "1");
        ...
        sessionStorage.removeItem("clipai_abrir_editor_google");
        googleLoginButton.disabled = false;
        authMessage.textContent = erro.message || "Não foi possível entrar com o Google.";
    }
});

if (sessionStorage.getItem("clipai_abrir_editor_google") === "1") {
    sessionStorage.removeItem("clipai_abrir_editor_google");
    abrirEditorDepoisDoLogin = true;
}
